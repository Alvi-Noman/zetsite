...
export const trustBadgesSchema: SectionSchema = {
  ...
  fields: [{ key: 'backgroundColor', type: 'color', label: 'Background color', default: '#F5F3FF', tab: 'style' }],
  defaultSettings: { backgroundColor: '#F5F3FF' },
};
...
export function TrustBadges({ settings, blocks }: SectionComponentProps<TrustBadgesSettings>) {
  ...
  return (
    <div className="px-4 py-8" style={{ backgroundColor: settings.backgroundColor }}>
      <div className="mx-auto grid max-w-5xl grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
        {badges.map((badge, i) => {
          const Icon = ICONS[badge.icon] ?? Shield;
          return (
            <div key={i} className="flex items-center gap-2.5 rounded-2xl bg-white px-3.5 py-3.5 shadow-sm ring-1 ring-violet-100 transition-shadow hover:shadow-md">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#ff71cd] to-[#9b51e0] text-white">
