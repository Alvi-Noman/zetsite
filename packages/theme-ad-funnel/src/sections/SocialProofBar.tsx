...
export function SocialProofBar({ settings, blocks }: SectionComponentProps<SocialProofBarSettings>) {
  ...
  if (settings.displayStyle === 'floating') {
    return (
      <div className={`pointer-events-none fixed bottom-4 z-40 ${POSITION_CLASS[settings.position] ?? POSITION_CLASS['bottom-left']}`}>
        ...
        >
          <p className="text-xs font-semibold text-neutral-700">{messages[index]?.message}</p>
  ...
  return (
    <div
      className={`relative bg-neutral-50 border-y border-neutral-200 px-4 py-2 text-xs font-semibold text-neutral-700 ${TEXT_ALIGN_CLASS[settings.align] ?? 'text-center'}`}
    >
