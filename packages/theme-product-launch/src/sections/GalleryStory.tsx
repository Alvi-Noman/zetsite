      ...
      <div className="flex flex-col gap-12">
        {images.map((img, i) => (
          <div key={i} className={`flex flex-col items-center gap-6 sm:flex-row ${i % 2 === 1 ? 'sm:flex-row-reverse' : ''}`}>
            {img.imageUrl ? (
              <ResponsiveImage src={img.imageUrl} alt="" className="aspect-[4/3] w-full rounded-lg object-cover sm:w-1/2" priority={priority && i === 0} />
            ) : (
              <div className="aspect-[4/3] w-full rounded-lg bg-neutral-100 sm:w-1/2" />
            )}
