...
export function HeroFullBleed({ settings, renderBlocks }: SectionComponentProps<HeroFullBleedSettings>) {
  const overlay = `linear-gradient(to top, rgba(0,0,0,${Math.min(1, settings.overlayOpacity + 0.25)}), rgba(0,0,0,${settings.overlayOpacity}) 40%, rgba(0,0,0,0.05))`;
  return (
    <section className="relative flex min-h-[420px] items-end overflow-hidden px-6 py-10 sm:min-h-[520px]">
      {settings.imageUrl ? (
        ...
        <div className="absolute inset-0 bg-neutral-800" style={{ backgroundImage: overlay }} />
      )}
