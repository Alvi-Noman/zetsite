  ...
  return (
    <div className={`relative flex h-full flex-col justify-center px-6 ${align}`}>
      <div className={`mx-auto w-full ${widthCls}`}>
        <h1 className="text-4xl font-serif font-normal tracking-tight max-w-2xl">{slide.heading}</h1>
        {slide.subheading ? <p className="mt-4 text-lg max-w-xl opacity-80">{slide.subheading}</p> : null}
        {slide.buttonText ? (
          <a
            href={slide.buttonUrl || '/'}
            className="mt-8 inline-block px-6 py-3 bg-yellow-700 text-white text-sm font-semibold rounded-md hover:bg-yellow-800 transition-colors"
          >
            {slide.buttonText}
          </a>
        ) : null}
...
export function HeroSlideshow({ settings, blocks }: SectionComponentProps<HeroSlideshowSettings>) {
  ...
  const widthCls = widthClass(settings.width, 'max-w-2xl mx-auto');
  ...
  return (
    <section
      className={`relative overflow-hidden bg-neutral-100 text-white ${heightCls}`}
    ...
    >
      ...
      {showArrows ? (
        <>
          <button
            ...
            className="absolute left-4 top-1/2 z-10 -translate-y-1/2 rounded-full bg-white/20 p-2 text-white hover:bg-white/30 transition-colors"
          >
            <ChevronLeft size={20} />
          </button>
          <button
            ...
            className="absolute right-4 top-1/2 z-10 -translate-y-1/2 rounded-full bg-white/20 p-2 text-white hover:bg-white/30 transition-colors"
          >
      ...
      {slides.length > 1 && settings.indicatorStyle === 'counter' ? (
        <div className="absolute bottom-6 right-6 z-10 rounded-full bg-black/30 px-3 py-1 text-xs font-semibold text-white">
        ...
        <div className="absolute inset-x-0 bottom-6 z-10 flex justify-center gap-2">
          {slides.map((_, i) => (
            <button
              ...
              type="button"
              onClick={() => goTo(i)}
              className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold transition-colors ${i === index ? 'bg-white text-neutral-900' : 'bg-white/20 text-white'}`}
