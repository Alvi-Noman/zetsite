...
export function RatingBadge({ settings }: SectionComponentProps<RatingBadgeSettings>) {
  ...
  return (
    <div className={`flex flex-col gap-1.5 px-4 pb-4 ${align}`}>
      <div className="inline-flex items-center gap-2 self-center rounded-full bg-stone-50 px-4 py-2 ring-1 ring-stone-200">
        <span className="flex items-center gap-0.5">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star
              key={i}
              size={15}
              className={i < Math.round(rating) ? 'fill-yellow-600 text-yellow-600' : 'fill-neutral-200 text-neutral-200'}
            />
          ))}
        </span>
