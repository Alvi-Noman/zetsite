...
export function HeroVideo({ settings, renderBlocks }: SectionComponentProps<HeroVideoSettings>) {
  ...
  return (
    <section className="relative flex flex-col justify-center gap-5 px-6 py-32 overflow-hidden bg-black text-white">
      {settings.videoUrl ? (
        <video
      ...
      <div className={`relative flex flex-col gap-5 w-full ${align} ${widthClass(settings.width, 'max-w-2xl mx-auto')}`}>
        {renderBlocks?.()}
      </div>
    </section>
  );
