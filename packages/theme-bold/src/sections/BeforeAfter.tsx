...
export function BeforeAfter({ settings, renderBlocks, priority }: SectionComponentProps<BeforeAfterSettings>) {
  ...
  if (!settings.beforeImageUrl || !settings.afterImageUrl) {
    return <section className={`px-6 py-20 mx-auto text-center text-sm text-neutral-400 bg-white ${widthCls}`}>Add a before and an after image</section>;
  }

  return (
    <section className={`px-6 py-20 mx-auto bg-white ${widthCls}`}>
      <div className={`flex flex-col mb-10 ${align}`}>{renderBlocks?.()}</div>
      <div className="relative aspect-video overflow-hidden border border-black select-none">
        ...
        {settings.afterLabel ? (
          <span className="absolute right-3 top-3 bg-black px-2 py-1 text-xs font-bold uppercase tracking-wide text-white">{settings.afterLabel}</span>
        ) : null}
        <div className="absolute inset-0 overflow-hidden" style={{ width: `${split}%` }}>
          ...
          {settings.beforeLabel ? (
            <span className="absolute left-3 top-3 bg-black px-2 py-1 text-xs font-bold uppercase tracking-wide text-white">{settings.beforeLabel}</span>
          ) : null}
        </div>
        <div className="absolute inset-y-0" style={{ left: `${split}%` }}>
          <div className="h-full w-0.5 bg-white" />
        </div>
        <input type="range" min={0} max={100} value={split} onChange={(e) => setSplit(Number(e.target.value))} className="absolute inset-x-0 bottom-3 mx-auto w-3/4" />
      </div>
    </section>
  );
}
