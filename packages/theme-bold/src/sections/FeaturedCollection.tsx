...
function ProductCard({
  ...
  return (
    <a href={`/products/${product.handle}`} className={`group block border border-black ${soldOut ? 'opacity-60' : ''}`}>
      <div className={`relative bg-neutral-100 overflow-hidden ${aspectClass || 'aspect-square'}`}>
        {image ? (
          <ResponsiveImage
            src={image}
            alt={product.title}
            priority={priority}
            className={`w-full h-full object-cover grayscale transition-all duration-200 ${secondaryImage ? 'group-hover:opacity-0' : 'group-hover:grayscale-0'}`}
          />
        ) : null}
        {secondaryImage ? (
          <ResponsiveImage
        ...
        {soldOut ? (
          <span className="absolute left-2 top-2 bg-black px-2 py-1 text-xs font-bold uppercase tracking-wide text-white">Sold out</span>
        ) : null}
      </div>
      <div className="p-3">
...
export function FeaturedCollection({
  ...
  return (
    <section className="px-6 py-20" style={{ backgroundColor: settings.backgroundColor }}>
      <div className={`flex flex-col mb-12 space-y-4 ${align}`}>
        {titleBlock ? (
          <h2 className="text-3xl font-black uppercase tracking-tight">{titleText}</h2>
        ) : null}
      ...
      {products.length === 0 ? (
        ...
        <div
          ...
          style={{ gap: settings.gap && settings.gap > 1 ? settings.gap : 16 }}
        >
          {products.map((p, i) => (
            <div
              key={p.id}
              className="w-1/2 shrink-0 snap-start md:w-auto"
              style={{ flexBasis: `calc((100% - ${((Number(settings.columns) || 4) - 1)} * ${settings.gap && settings.gap > 1 ? settings.gap : 16}px) / ${settings.columns || 4})` }}
            >
              <ProductCard
                product={p}
        ...
        <div
          className={`grid ${mobileCols} ${desktopCols} bg-black mx-auto ${widthClass(settings.width, 'max-w-5xl')}`}
