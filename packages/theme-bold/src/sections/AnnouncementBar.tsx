...
export function AnnouncementBar({ settings, blocks }: SectionComponentProps<AnnouncementBarSettings>) {
  ...
  const content = <span className="text-xs font-bold uppercase tracking-widest">{item.text}</span>;

  return (
    <div
      className={`relative px-4 py-2.5 ${settings.showDivider ? 'border-b border-white/20' : ''} ${TEXT_ALIGN_CLASS[settings.align] ?? 'text-center'}`}
