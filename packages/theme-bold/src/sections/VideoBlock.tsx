...
export function VideoBlock({ settings, renderBlocks }: SectionComponentProps<VideoBlockSettings>) {
  ...
  return (
    <section className={`px-6 py-20 mx-auto bg-white ${widthClass(settings.width, 'max-w-4xl')}`}>
      ...
      {!settings.videoUrl ? (
        <div className="aspect-video border border-black flex items-center justify-center text-neutral-400 text-sm">
          Add a video URL
        </div>
      ) : isLightbox ? (
        <>
          <button
            ...
            className="group relative block aspect-video w-full overflow-hidden border border-black bg-black"
            aria-label="Play video"
          >
            {thumbnail ? (
              <ResponsiveImage src={thumbnail} alt="" className="h-full w-full object-cover grayscale opacity-80 transition-opacity group-hover:opacity-100" />
            ) : null}
