...
function Stars({ rating }: { rating: number }) {
  return <div className="text-sm mb-2">{'★'.repeat(Math.max(0, Math.min(5, rating))).padEnd(5, '☆')}</div>;
}
...
export function Testimonials({ settings, blocks, renderBlocks }: SectionComponentProps<TestimonialsSettings>) {
  ...
  return (
    <section className={`px-6 py-20 mx-auto text-center bg-white ${widthClass(settings.width, 'max-w-3xl')}`}>
      <div className="mb-12">{renderBlocks?.((b) => b.type === 'heading')}</div>
      {item ? (
        <>
          <Stars rating={item.rating ?? 5} />
          <p className="text-xl text-neutral-800 font-medium">&ldquo;{item.quote}&rdquo;</p>
          <div className="mt-6 flex items-center justify-center gap-2">
            {item.avatarUrl ? <ResponsiveImage src={item.avatarUrl} alt="" className="h-8 w-8 object-cover" /> : null}
            <span className="text-xs font-bold uppercase tracking-wide text-neutral-900">{item.author}</span>
          </div>
          {items.length > 1 && (
            <div className="mt-8 flex justify-center gap-2">
              {items.map((_, i) => (
                <button
                  ...
                  type="button"
                  onClick={() => setIndex(i)}
                  className={`h-1 w-8 transition-colors ${i === index ? 'bg-black' : 'bg-neutral-300'}`}
