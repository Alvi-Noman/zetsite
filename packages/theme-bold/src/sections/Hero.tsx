...
export const heroSchema: SectionSchema = {
  ...
  defaultBlocks: [
    ...
    { type: 'button', settings: { text: 'Shop now', url: '/' } },
  ],
  fields: [
    ...
    { key: 'overlayOpacity', type: 'number', label: 'Image overlay opacity (0-1)', default: 0.55, tab: 'style' },
    {
      key: 'textAlign',
      type: 'select',
...
export function Hero({ settings, renderBlocks }: SectionComponentProps<HeroSettings>) {
  ...
  return (
    <section
      className={`relative flex flex-col ${verticalClass} gap-5 overflow-hidden px-6 py-20 text-white ${mobileAlign} ${desktopAlign} ${HEIGHT_CLASS[settings.height ?? 'medium']}`}
    ...
    >
      ...
      <div className={`relative w-full ${widthClass(settings.width, 'max-w-2xl mx-auto')} ${settings.showTextBackdrop ? TEXT_BACKDROP_CLASS : ''}`}>
        {renderBlocks?.()}
      </div>
    </section>
  );
