...
export function SocialProofBar({ settings, blocks }: SectionComponentProps<SocialProofBarSettings>) {
  ...
  if (settings.displayStyle === 'floating') {
    return (
      <div className={`pointer-events-none fixed bottom-4 z-40 ${POSITION_CLASS[settings.position] ?? POSITION_CLASS['bottom-left']}`}>
        ...
        <div
          key={index}
          className="pointer-events-auto flex max-w-xs items-center gap-3 border border-black bg-white px-4 py-3"
          style={{ animation: 'zetsite-proof-in 0.4s ease-out' }}
        >
          <p className="text-xs font-bold uppercase tracking-wide text-neutral-900">{messages[index]?.message}</p>
          {settings.dismissible ? (
            <button type="button" onClick={() => setDismissed(true)} aria-label="Dismiss" className="shrink-0 text-neutral-400 hover:text-black">
              <X size={14} />
            </button>
          ) : null}
        </div>
  ...
  return (
    <div
      className={`relative bg-black text-white px-4 py-2.5 text-xs font-bold uppercase tracking-widest ${TEXT_ALIGN_CLASS[settings.align] ?? 'text-center'}`}
    >
