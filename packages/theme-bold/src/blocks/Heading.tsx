...
const SIZE_CLASS: Record<HeadingSettings['size'], string> = {
  sm: 'text-lg font-black uppercase tracking-widest',
  md: 'text-3xl font-black uppercase tracking-tight',
  lg: 'text-5xl font-black uppercase tracking-tight',
};
...
export function Heading({ settings, onFieldChange }: SectionComponentProps<HeadingSettings>) {
  return (
    <Editable
      ...
      className={`${SIZE_CLASS[settings.size] ?? SIZE_CLASS.md} text-current block`}
    />
  );
}
