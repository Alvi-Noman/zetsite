...
const ORDER_PANEL_CLASS_NAMES = {
  button: 'px-8 py-3 bg-neutral-900 text-white text-sm font-medium rounded-md hover:bg-neutral-800 transition-colors disabled:opacity-40 disabled:cursor-not-allowed',
  accentText: 'text-neutral-900',
  accentFocus: 'focus:border-neutral-900',
};
...
function StockIndicator({ variant }: { variant: StorefrontVariant | undefined }) {
  ...
  if (variant.available <= 0) return <p className="mt-2 text-sm font-medium text-red-600">Out of stock</p>;
  if (variant.available <= 5) {
    return <p className="mt-2 text-sm font-medium text-amber-600">Only {variant.available} left in stock</p>;
  }
...
export function ProductTemplate({ storeSlug, handle }: ProductTemplateProps) {
  ...
  if (product === undefined) {
    return <div className="px-6 py-16 text-center text-neutral-400">Loading…</div>;
  }
  if (product === null) {
    return <div className="px-6 py-16 text-center text-neutral-400">Product not found</div>;
  }
  ...
  return (
    <div className="pb-16">
              ...
              {product.variants.map((v, i) => (
                <button
                  ...
                  className={`rounded-md border px-3 py-1.5 text-sm ${
                    i === variantIndex ? 'border-neutral-900 bg-neutral-900 text-white' : 'border-neutral-300 text-neutral-700'
                  }`}
                >
                  {v.label}
                </button>
      ...
      {related.length > 0 && (
        <div className="px-6 py-12 max-w-5xl mx-auto border-t border-neutral-200">
          <h2 className="text-xl font-semibold text-neutral-900 mb-6">You may also like</h2>
            ...
            {related.map((p) => {
              ...
              return (
                <a key={p.id} href={`/products/${p.handle}`} className="group block">
                  ...
                  <p className="mt-3 text-sm font-medium text-neutral-900">{p.title}</p>
                </a>
              );
            })}
          </div>
      ...
      {stickyVisible && (
        <div className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-between gap-4 border-t border-neutral-200 bg-white px-6 py-3 shadow-lg">
          ...
          <button
            ...
            className="px-6 py-2.5 bg-neutral-900 text-white text-sm font-medium rounded-md hover:bg-neutral-800 disabled:opacity-40"
          >
