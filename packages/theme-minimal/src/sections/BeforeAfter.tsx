...
export function BeforeAfter({ settings, renderBlocks, priority }: SectionComponentProps<BeforeAfterSettings>) {
  ...
  if (!settings.beforeImageUrl || !settings.afterImageUrl) {
    return <section className={`px-6 py-16 mx-auto text-center text-sm text-neutral-400 ${widthCls}`}>Add a before and an after image</section>;
  }

  return (
    <section className={`px-6 py-16 mx-auto ${widthCls}`}>
      ...
      <div className="relative aspect-video overflow-hidden rounded-md select-none">
        ...
        {settings.afterLabel ? (
          <span className="absolute right-3 top-3 rounded-md bg-black/60 px-2 py-1 text-xs font-medium text-white">{settings.afterLabel}</span>
        ) : null}
        <div className="absolute inset-0 overflow-hidden" style={{ width: `${split}%` }}>
          ...
          {settings.beforeLabel ? (
            <span className="absolute left-3 top-3 rounded-md bg-black/60 px-2 py-1 text-xs font-medium text-white">{settings.beforeLabel}</span>
          ) : null}
        </div>
