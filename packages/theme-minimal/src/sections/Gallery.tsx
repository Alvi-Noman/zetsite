...
export function Gallery({ settings, blocks, renderBlocks, priority }: SectionComponentProps<GallerySettings>) {
  ...
  return (
    <section className={`px-6 py-16 mx-auto ${widthClass(settings.width, 'max-w-5xl')}`}>
      ...
      {images.length === 0 ? (
        ...
        <div className={MASONRY_COLUMN_CLASS[settings.columns] ?? MASONRY_COLUMN_CLASS['3']} style={{ columnGap: gap }}>
          {images.map((img, i) =>
            enableLightbox ? (
              <button
                key={i}
                ...
                className="mb-4 block w-full break-inside-avoid overflow-hidden rounded-md bg-neutral-100"
                style={{ marginBottom: gap }}
              >
                <ResponsiveImage src={img.imageUrl} alt={img.caption ?? ''} className="w-full object-cover hover:scale-105 transition-transform" priority={priority && i === 0} />
              </button>
            ) : (
              <div key={i} className="break-inside-avoid overflow-hidden rounded-md bg-neutral-100" style={{ marginBottom: gap }}>
                <ResponsiveImage src={img.imageUrl} alt={img.caption ?? ''} className="w-full object-cover" priority={priority && i === 0} />
              </div>
            ),
          )}
        </div>
      ) : (
        <div className={`grid ${COLUMN_CLASS[settings.columns] ?? COLUMN_CLASS['3']}`} style={{ gap }}>
          {images.map((img, i) =>
            enableLightbox ? (
              <button
                key={i}
                type="button"
                onClick={() => setLightbox(i)}
                className={`overflow-hidden rounded-md bg-neutral-100 ${aspectClass || 'aspect-square'}`}
              >
                <ResponsiveImage src={img.imageUrl} alt={img.caption ?? ''} className="h-full w-full object-cover hover:scale-105 transition-transform" priority={priority && i === 0} />
              </button>
            ) : (
              <div key={i} className={`overflow-hidden rounded-md bg-neutral-100 ${aspectClass || 'aspect-square'}`}>
                <ResponsiveImage src={img.imageUrl} alt={img.caption ?? ''} className="h-full w-full object-cover" priority={priority && i === 0} />
              </div>
            ),
          )}
        </div>
      ...
      {enableLightbox && lightbox !== null && images[lightbox] && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-6" onClick={() => setLightbox(null)}>
          ...
          <img src={images[lightbox].imageUrl} alt={images[lightbox].caption} className="max-h-[85vh] max-w-full rounded-md" />
        </div>
      )}
    </section>
  );
