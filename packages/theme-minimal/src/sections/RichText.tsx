...
export function RichText({ settings, blocks, renderBlocks }: SectionComponentProps<RichTextSettings>) {
  ...
  const textBlock = (
    <div className={`flex-1 space-y-4 flex flex-col ${align} ${hasImage ? `${mobileTextOrder} md:order-none` : ''}`}>
      {settings.eyebrow ? (
        <span className="text-xs font-semibold uppercase tracking-widest text-neutral-500">{settings.eyebrow}</span>
      ) : null}
  ...
  const imageBlock = hasImage ? (
    <div className={`flex-1 overflow-hidden rounded-md ${mobileImageOrder} md:order-none`} style={zoomStyle}>
      {renderBlocks?.((b) => b.type === 'image')}
    </div>
  ) : null;
  ...
  return (
    <section
      className={`px-6 py-16 mx-auto ${widthClass(settings.width, 'max-w-5xl')}`}
      style={{ backgroundColor: settings.backgroundColor }}
    >
      {zoomStyle ? <style>{AMBIENT_ZOOM_KEYFRAMES}</style> : null}
      <div className={`flex flex-col md:flex-row ${rowAlign}`} style={{ gap: settings.gap ?? 40 }}>
