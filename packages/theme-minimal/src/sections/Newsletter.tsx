...
export function Newsletter({ settings, storeSlug, onFieldChange, renderBlocks }: SectionComponentProps<NewsletterSettings>) {
  ...
  return (
    <section className={`px-6 py-16 flex flex-col ${align}`} style={{ backgroundColor: settings.backgroundColor }}>
      <div className="space-y-2 max-w-md">{renderBlocks?.()}</div>
      {status === 'sent' ? (
        <p className="mt-6 text-sm font-medium text-neutral-900">{settings.successMessage || 'Thanks for subscribing!'}</p>
      ) : (
