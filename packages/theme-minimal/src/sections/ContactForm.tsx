...
export function ContactForm({ settings, storeSlug, onFieldChange, blocks, renderBlocks }: SectionComponentProps<ContactFormSettings>) {
  ...
  return (
    <section className={`px-6 py-16 mx-auto flex flex-col ${align} ${widthClass(settings.width, 'max-w-lg')}`}>
      <div className="mb-8 w-full text-center">{renderBlocks?.((b) => b.type === 'heading')}</div>
      {status === 'sent' ? (
        <p className="text-center text-sm font-medium text-neutral-900">{settings.successMessage || "Thanks — we'll be in touch."}</p>
      ) : (
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          ...
          <button type="submit" className="w-full rounded-md bg-neutral-900 px-4 py-2.5 text-sm font-medium text-white hover:bg-neutral-800">
