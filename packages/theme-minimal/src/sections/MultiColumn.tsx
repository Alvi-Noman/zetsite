...
export function MultiColumn({ settings, blocks, renderBlocks }: SectionComponentProps<MultiColumnSettings>) {
  ...
  return (
    <section className={`px-6 py-16 mx-auto ${widthClass(settings.width, 'max-w-5xl')}`}>
      ...
      <div
        className="grid"
        style={{ gridTemplateColumns: `repeat(${Math.max(1, settings.columns || 3)}, minmax(0, 1fr))`, gap: settings.gap ?? 32 }}
      >
        {columns.map((col, i) => {
          ...
          const content = (
            <div className="text-center">
              ...
              <h3 className="text-base font-semibold text-neutral-900">{col.heading}</h3>
      ...
      {settings.buttonText ? (
        <div className={`mt-10 flex ${settings.align === 'left' ? 'justify-start' : settings.align === 'right' ? 'justify-end' : 'justify-center'}`}>
          <a
            ...
            className="inline-block rounded-md bg-neutral-900 px-6 py-3 text-sm font-medium text-white hover:bg-neutral-800 transition-colors"
          >
            {settings.buttonText}
          </a>
        </div>
