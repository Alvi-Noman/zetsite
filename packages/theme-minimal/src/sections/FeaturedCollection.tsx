  ...
  return (
    <a href={`/products/${product.handle}`} className={`group block ${soldOut ? 'opacity-60' : ''}`}>
      <div className={`relative bg-neutral-100 overflow-hidden rounded-md ${aspectClass || 'aspect-square'}`}>
        ...
        {soldOut ? (
          <span className="absolute left-2 top-2 rounded-md bg-neutral-900 px-2 py-1 text-xs font-medium text-white">Sold out</span>
        ) : null}
      </div>
      <p className="mt-3 text-sm font-medium text-neutral-900">{product.title}</p>
      {product.price != null ? (
...
export function FeaturedCollection({
  ...
  return (
    <section className="px-6 py-16" style={{ backgroundColor: settings.backgroundColor }}>
      <div className={`flex flex-col mb-10 space-y-3 ${align}`}>
        {titleBlock ? (
          <h2 className="text-2xl font-semibold text-neutral-900">{titleText}</h2>
        ) : null}
