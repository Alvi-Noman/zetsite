  ...
  return (
    <div className={`relative flex h-full flex-col justify-center px-6 ${align}`}>
      <div className={`mx-auto w-full ${widthCls}`}>
        <h1 className="text-4xl font-semibold tracking-tight max-w-2xl">{slide.heading}</h1>
        {slide.subheading ? <p className="mt-4 text-lg max-w-xl opacity-80">{slide.subheading}</p> : null}
        {slide.buttonText ? (
          <a
            href={slide.buttonUrl || '/'}
            className="mt-8 inline-block px-6 py-3 bg-neutral-900 text-white text-sm font-medium rounded-md hover:bg-neutral-800 transition-colors"
          >
            {slide.buttonText}
          </a>
        ) : null}
  ...
  return (
    ...
    >
      ...
      {slides.length > 1 && settings.indicatorStyle === 'counter' ? (
        <div className="absolute bottom-6 right-6 z-10 rounded-full bg-black/30 px-3 py-1 text-xs font-medium text-white">
        ...
        <div className="absolute inset-x-0 bottom-6 z-10 flex justify-center gap-2">
          {slides.map((_, i) => (
            <button
              ...
              type="button"
              onClick={() => goTo(i)}
              className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-medium transition-colors ${i === index ? 'bg-white text-neutral-900' : 'bg-white/20 text-white'}`}
