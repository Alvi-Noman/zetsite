...
const ORDER_PANEL_CLASS_NAMES = {
  button:
    'px-10 py-3.5 bg-violet-600 text-white text-sm font-bold rounded-full hover:bg-violet-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed',
  accentText: 'text-violet-600',
  accentFocus: 'focus:border-violet-600',
};
...
function StockIndicator({ variant }: { variant: StorefrontVariant | undefined }) {
  ...
  if (variant.available <= 0) return <p className="mt-2 text-sm font-medium text-violet-600">Out of stock</p>;
              ...
              {TABS.map((t) => (
                <button
                  ...
                  className={`py-3 text-sm font-semibold border-b-2 -mb-px ${
                    tab === t ? 'border-violet-600 text-violet-600' : 'border-transparent text-neutral-400'
                  }`}
                >
                  {t}
                </button>
      ...
      {stickyVisible && (
        <div className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-between gap-4 border-t border-neutral-200 bg-white px-6 py-3 shadow-lg">
          ...
          <button
            ...
            className="px-6 py-2.5 bg-violet-600 text-white text-sm font-bold rounded-full hover:bg-violet-700 disabled:opacity-40"
          >
