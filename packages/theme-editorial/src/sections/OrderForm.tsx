  ...
  if (status === 'sent') {
    return (
      <section id="order" className="mx-auto max-w-3xl px-4 py-12">
        <p className="rounded-lg border border-emerald-100 bg-emerald-50 px-6 py-8 text-center text-sm font-semibold text-emerald-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 lg:col-span-3">
          ...
          {hasVariants && product!.variants.length > 1 ? (
            <div>
                ...
                {product!.variants.map((v, i) => (
                  <button
                    ...
                    className={`rounded-md border px-3 py-1.5 text-sm ${
                      i === variantIndex ? 'border-emerald-700 bg-emerald-700 text-white' : 'border-neutral-300 text-neutral-700'
                    }`}
                  >
                    {v.label}
                  </button>
              ...
              {outOfStock ? <p className="mt-1.5 text-xs font-medium text-emerald-700">Out of stock</p> : null}
            </div>
          ) : null}
          ...
          <label className="block">
            ...
            <input
              ...
              className="w-full rounded-md border border-neutral-300 px-3 py-2.5 text-sm focus:border-emerald-600 focus:outline-none"
            />
          </label>
          <label className="block">
                ...
                {shippingOptions.map((opt, i) => (
                  <label
                    ...
                    className={`flex cursor-pointer items-center justify-between rounded-md border px-3 py-2.5 text-sm ${
                      i === shippingIndex ? 'border-emerald-600 bg-emerald-50' : 'border-neutral-300'
                    }`}
                  >
          ...
          {status === 'error' && error ? <p className="text-sm font-medium text-emerald-700">{error}</p> : null}

          <button
            type="submit"
            disabled={status === 'submitting' || outOfStock}
            className="w-full rounded-md bg-emerald-700 px-4 py-3 text-sm font-bold text-white shadow-sm transition-colors hover:bg-emerald-800 disabled:opacity-60"
          >
        ...
        <div className="lg:col-span-2">
          <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-5">
            ...
            <div className="flex justify-between border-t border-neutral-200 pt-4">
              ...
              <dd className="text-lg font-black text-emerald-700">{money(currency, total)}</dd>
            </div>
          </div>
        </div>
      </div>
