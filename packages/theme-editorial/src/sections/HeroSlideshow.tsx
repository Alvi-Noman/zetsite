  ...
  return (
    <div className={`relative flex h-full flex-col justify-center px-6 ${align}`}>
      <div className={`mx-auto w-full ${widthCls}`}>
        <h1 className="text-4xl font-bold tracking-tight max-w-2xl">{slide.heading}</h1>
        {slide.subheading ? <p className="mt-4 text-lg max-w-xl opacity-80">{slide.subheading}</p> : null}
        {slide.buttonText ? (
          <a
            href={slide.buttonUrl || '/'}
            className="mt-8 inline-block px-6 py-3 bg-emerald-700 text-white text-sm font-semibold rounded-md hover:bg-emerald-800 transition-colors"
          >
            {slide.buttonText}
          </a>
        ) : null}
