...
const ORDER_PANEL_CLASS_NAMES = {
  button:
    'px-10 py-3.5 bg-emerald-700 text-white text-sm font-bold rounded-full hover:bg-emerald-800 transition-colors disabled:opacity-40 disabled:cursor-not-allowed',
  accentText: 'text-emerald-700',
  accentFocus: 'focus:border-emerald-700',
};
...
function StockIndicator({ variant }: { variant: StorefrontVariant | undefined }) {
  ...
  if (variant.available <= 0) return <p className="mt-2 text-sm font-medium text-emerald-700">Out of stock</p>;
              ...
              {TABS.map((t) => (
                <button
                  ...
                  className={`py-3 text-sm font-semibold border-b-2 -mb-px ${
                    tab === t ? 'border-emerald-700 text-emerald-700' : 'border-transparent text-neutral-400'
                  }`}
                >
                  {t}
                </button>
      ...
      {stickyVisible && (
        <div className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-between gap-4 border-t border-neutral-200 bg-white px-6 py-3 shadow-lg">
          ...
          <button
            ...
            className="px-6 py-2.5 bg-emerald-700 text-white text-sm font-bold rounded-full hover:bg-emerald-800 disabled:opacity-40"
          >
