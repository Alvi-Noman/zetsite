...
const SIZE_CLASS: Record<HeadingSettings['size'], string> = {
  ...
  md: 'text-2xl font-normal sm:text-3xl',
  lg: 'text-4xl font-normal sm:text-5xl',
};
...
export function Heading({ settings, onFieldChange }: SectionComponentProps<HeadingSettings>) {
  return (
    <Editable
      ...
      className={`${SIZE_CLASS[settings.size] ?? SIZE_CLASS.md} font-serif tracking-tight text-current block`}
    />
  );
}
