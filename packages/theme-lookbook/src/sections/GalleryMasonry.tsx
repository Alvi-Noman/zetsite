  ...
  return (
    <section className="mx-auto max-w-4xl px-6 py-10">
      ...
      <div className="columns-2 gap-3 sm:columns-3">
        {images.map((img, i) =>
          img.imageUrl ? (
            <ResponsiveImage key={i} src={img.imageUrl} alt="" className={`mb-3 w-full break-inside-avoid object-cover ${HEIGHTS[i % HEIGHTS.length]}`} priority={priority && i === 0} />
          ) : (
            <div key={i} className={`mb-3 w-full break-inside-avoid bg-neutral-100 ${HEIGHTS[i % HEIGHTS.length]}`} />
          ),
        )}
      </div>
    </section>
