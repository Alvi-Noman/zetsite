...
export function GalleryCarousel({ blocks, renderBlocks, priority }: SectionComponentProps<GalleryCarouselSettings>) {
  ...
  return (
    <section className="mx-auto max-w-2xl px-6 py-10">
      ...
      {current.imageUrl ? (
        <ResponsiveImage src={current.imageUrl} alt="" className="aspect-[4/5] w-full object-cover" priority={priority} />
      ) : (
        ...
        <div className="mt-4 flex justify-center gap-2">
          {images.map((_, i) => (
            <button
              ...
              className={`h-1.5 w-6 rounded-full transition-colors ${i === index ? 'bg-stone-900' : 'bg-neutral-300'}`}
            />
          ))}
        </div>
      ) : null}
