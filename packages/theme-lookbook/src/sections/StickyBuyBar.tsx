...
export function StickyBuyBar({ settings }: SectionComponentProps<StickyBuyBarSettings>) {
  ...
  return (
    <div
      className={`fixed inset-x-0 bottom-0 z-40 border-t border-yellow-100 bg-white/95 shadow-[0_-4px_16px_rgba(0,0,0,0.08)] backdrop-blur ${
      ...
      <div className="mx-auto flex w-full max-w-5xl items-center justify-between gap-3 px-4 py-2.5">
        <div className="min-w-0">
          <div className="flex items-baseline gap-2">
            <span className="text-lg font-black text-yellow-700">{settings.priceText}</span>
        ...
        <a
          ...
          className="shrink-0 rounded-full bg-yellow-700 px-6 py-2.5 text-sm font-bold text-white shadow-sm transition-colors hover:bg-yellow-800"
        >
          {settings.buttonText}
        </a>
      </div>
