...
export interface SwatchesPillsSettings {
  heading: string;
}

export const swatchesPillsSchema: SectionSchema = {
  type: 'swatchesPills',
  label: 'Variant swatches (pill buttons)',
...
export function SwatchesPills({ settings, blocks }: SectionComponentProps<SwatchesPillsSettings>) {
          ...
          {swatches.map((swatch, i) => (
            <button
              ...
              className={`rounded-full border px-4 py-1.5 text-sm font-medium transition-colors ${
                i === selected ? 'border-stone-900 bg-stone-900 text-white' : 'border-neutral-300 text-neutral-700 hover:border-stone-500'
              }`}
            >
              {swatch.label}
            </button>
