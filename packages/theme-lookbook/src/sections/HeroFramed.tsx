...
// The one design in this family that's fully contained — a bordered,
// shadowed card centered on the page — distinct from Split (image beside
// copy), Centered/Full-bleed (image as backdrop), and Image first/Minimal
// (no card boundary at all).
export function HeroFramed({ settings, renderBlocks, priority }: SectionComponentProps<HeroFramedSettings>) {
  return (
    <section className="px-6 py-14 sm:py-20" style={{ backgroundColor: settings.backgroundColor }}>
      <div className="mx-auto max-w-md overflow-hidden border border-neutral-200 bg-white shadow-lg shadow-stone-900/5">
        {settings.imageUrl ? (
        ...
        <div className="flex flex-col items-center gap-3 p-6 text-center">
          {settings.badgeText ? (
            <span className="inline-block rounded-full bg-stone-900 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-white">
              {settings.badgeText}
            </span>
          ) : null}
          {renderBlocks?.()}
