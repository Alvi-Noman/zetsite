...
export interface FeaturedCollectionMasonrySettings {
  collectionHandle: string;
  limit: number;
  backgroundColor: string;
}

export const featuredCollectionMasonrySchema: SectionSchema = {
  type: 'featuredCollectionMasonry',
  label: 'Featured collection (masonry)',
  ...
  defaultSettings: { collectionHandle: '', limit: 6, backgroundColor: '#ffffff' },
};

const HEIGHTS = ['h-48', 'h-64', 'h-56', 'h-40'];

export function FeaturedCollectionMasonry({ settings, storeSlug, blocks, renderBlocks, priority }: SectionComponentProps<FeaturedCollectionMasonrySettings>) {
  ...
  return (
    <section className="px-6 py-10" style={{ backgroundColor: settings.backgroundColor }}>
      ...
      <div className="mx-auto columns-2 gap-3 sm:columns-3">
        {products.map((p, i) => (
          <a key={p.id} href={`/products/${p.handle}`} className="mb-3 block break-inside-avoid">
            <div className={`overflow-hidden bg-neutral-100 ${HEIGHTS[i % HEIGHTS.length]}`}>
