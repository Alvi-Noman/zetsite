...
export const collectionListSchema: SectionSchema = {
  ...
  defaultSettings: { columns: '3', align: 'center', width: 'page', gap: 24, imageAspect: 'landscape' },
};
  ...
  return (
    <section className="px-6 py-10">
      ...
      {collections.length === 0 ? (
        ...
        <div
          className={`grid ${COLUMN_CLASS[settings.columns] ?? COLUMN_CLASS['3']} mx-auto ${widthClass(settings.width, 'max-w-5xl')}`}
        ...
        >
          {collections.map((c) => (
            <a
              ...
              className={`flex items-center justify-center rounded-md bg-neutral-100 text-neutral-900 font-semibold hover:bg-neutral-200 transition-colors ${aspectClass || 'aspect-[4/3]'}`}
            >
              {c.name}
            </a>
          ))}
