...
export function GalleryThumbs({ blocks, renderBlocks, priority }: SectionComponentProps<GalleryThumbsSettings>) {
  ...
  return (
    <section className="mx-auto max-w-2xl px-6 py-10">
      ...
      {current.imageUrl ? (
        <ResponsiveImage src={current.imageUrl} alt="" className="aspect-square w-full object-cover" priority={priority} />
      ) : (
      ...
      <div className="mt-3 flex justify-center gap-2">
        {images.map((img, i) => (
          <button
            ...
            type="button"
            onClick={() => setIndex(i)}
            className={`h-14 w-14 shrink-0 overflow-hidden border-2 ${i === index ? 'border-stone-900' : 'border-transparent opacity-70'}`}
          >
