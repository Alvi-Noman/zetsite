  ...
  if (status === 'sent') {
    return (
      <section id="order" className="mx-auto max-w-3xl px-4 py-12">
        <p className="rounded-lg border border-amber-100 bg-amber-50 px-6 py-8 text-center text-sm font-semibold text-amber-700">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 lg:col-span-3">
          ...
          {hasVariants && product!.variants.length > 1 ? (
            <div>
              ...
              {outOfStock ? <p className="mt-1.5 text-xs font-medium text-amber-600">Out of stock</p> : null}
            </div>
          ) : null}
          ...
          <label className="block">
            ...
            <input
              ...
              className="w-full rounded-md border border-neutral-300 px-3 py-2.5 text-sm focus:border-amber-500 focus:outline-none"
            />
          </label>
          <label className="block">
                ...
                {shippingOptions.map((opt, i) => (
                  <label
                    ...
                    className={`flex cursor-pointer items-center justify-between rounded-md border px-3 py-2.5 text-sm ${
                      i === shippingIndex ? 'border-amber-500 bg-amber-50' : 'border-neutral-300'
                    }`}
                  >
          ...
          {status === 'error' && error ? <p className="text-sm font-medium text-amber-600">{error}</p> : null}

          <button
            type="submit"
            disabled={status === 'submitting' || outOfStock}
            className="w-full rounded-md bg-amber-600 px-4 py-3 text-sm font-bold text-white shadow-sm transition-colors hover:bg-amber-700 disabled:opacity-60"
          >
        ...
        <div className="lg:col-span-2">
          <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-5">
            ...
            <div className="flex justify-between border-t border-neutral-200 pt-4">
              ...
              <dd className="text-lg font-black text-amber-600">{money(currency, total)}</dd>
            </div>
          </div>
        </div>
      </div>
