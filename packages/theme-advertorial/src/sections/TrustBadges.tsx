...
export const trustBadgesSchema: SectionSchema = {
  ...
  fields: [{ key: 'backgroundColor', type: 'color', label: 'Background color', default: '#fef2f2', tab: 'style' }],
  defaultSettings: { backgroundColor: '#fef2f2' },
};
...
export function TrustBadges({ settings, blocks }: SectionComponentProps<TrustBadgesSettings>) {
  ...
  return (
    <div className="px-4 py-6 border-y border-red-100" style={{ backgroundColor: settings.backgroundColor }}>
      <div className="mx-auto grid max-w-5xl grid-cols-2 gap-4 sm:grid-cols-4">
        {badges.map((badge, i) => {
          const Icon = ICONS[badge.icon] ?? Shield;
          return (
            <div key={i} className="flex items-center gap-2.5 rounded-lg bg-white px-3 py-3 shadow-sm ring-1 ring-red-100">
