  ...
  if (status === 'sent') {
    return (
      <section id="order" className="mx-auto max-w-3xl px-4 py-12">
        <p className="rounded-lg border border-red-100 bg-red-50 px-6 py-8 text-center text-sm font-semibold text-red-700">
        ...
        <form onSubmit={handleSubmit} className="min-w-0 space-y-4 lg:col-span-3">
          ...
          {hasVariants && product!.variants.length > 1 ? (
            <div>
                ...
                {product!.variants.map((v, i) => (
                  <button
                    ...
                    className={`rounded-md border px-3 py-1.5 text-sm ${
                      i === variantIndex ? 'border-red-600 bg-red-600 text-white' : 'border-neutral-300 text-neutral-700'
                    }`}
                  >
                    {v.label}
                  </button>
              ...
              {outOfStock ? <p className="mt-1.5 text-xs font-medium text-red-600">Out of stock</p> : null}
            </div>
          ) : null}
                ...
                {shippingOptions.map((opt, i) => (
                  <label
                    ...
                    className={`flex cursor-pointer items-center justify-between rounded-md border px-3 py-2.5 text-sm ${
                      i === shippingIndex ? 'border-red-500 bg-red-50' : 'border-neutral-300'
                    }`}
                  >
          ...
          <button
            ...
            className="w-full rounded-md bg-red-600 px-4 py-3 text-sm font-bold text-white shadow-sm transition-colors hover:bg-red-700 disabled:opacity-60"
          >
        ...
        <div className="min-w-0 lg:col-span-2">
          <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-5">
            ...
            <div className="flex justify-between border-t border-neutral-200 pt-4">
              ...
              <dd className="text-lg font-black text-red-600">{money(currency, total)}</dd>
            </div>
          </div>
        </div>
      </div>
